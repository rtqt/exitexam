            ...
            <div className="max-w-6xl mx-auto">
                <div className="flex justify-between items-center mb-8 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <div>
                        ...
                        <p className="text-slate-500">Manage exam questions and content</p>
                    </div>
                    <div className="flex gap-4">
                        <button onClick={() => { if (window.confirm('Reset all questions to default?')) resetToDefault() }} className="text-red-500 underline text-sm">Reset DB</button>
                        <button
                            onClick={onExit}
                            className="bg-slate-800 text-white px-6 py-2 rounded-xl font-medium hover:bg-slate-900 transition-all"
                        >
                            Exit Admin
                        </button>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="md:col-span-1 space-y-2">
                        <button
                            ...
                            className={`w-full text-left px-4 py-3 rounded-xl font-medium flex items-center gap-3 transition-all ${activeTab === 'add' ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-100'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`w-full text-left px-4 py-3 rounded-xl font-medium flex items-center gap-3 transition-all ${activeTab === 'import' ? 'bg-purple-600 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-100'
                                }`}
                        >
                            <Brain className="w-5 h-5" /> AI Import (Gemini)
                        </button>
                        <button
                            onClick={() => setActiveTab('list')}
                            className={`w-full text-left px-4 py-3 rounded-xl font-medium flex items-center gap-3 transition-all ${activeTab === 'list' ? 'bg-slate-600 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-100'
                                }`}
                        >
                            <Save className="w-5 h-5" /> Manage Questions
                        </button>

                        <div className="bg-white p-4 rounded-xl mt-6 border border-slate-200">
                            <h3 className="font-bold text-slate-700 mb-2">Stats</h3>
                            <p className="text-sm text-slate-500">Total Questions: <span className="font-bold text-slate-800">{questions.length}</span></p>
                    ...
                    <div className="md:col-span-3">
                        ...
                        {activeTab === 'add' && (
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    {editingId && (
                                        <button onClick={cancelEdit} className="text-sm text-slate-500 hover:text-slate-700 underline">
                                            Cancel Editing
                                        </button>
                                    )}
                                </div>
                                <form onSubmit={handleManualAdd} className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-slate-700 mb-1">Theme / Subject</label>
                                            <input
                                                required
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            required
                                            className="w-full p-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none h-24"
                                            value={newQ.question}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {newQ.options.map((opt, i) => (
                                            <div key={i}>
                                                ...
                                                <input
                                                    required
                                                    className={`w-full p-2.5 rounded-lg border ${newQ.answer === i ? 'border-blue-500 bg-blue-50' : 'border-slate-300'} outline-none`}
                                                    value={opt}
                                                    onChange={e => {
                        ...
                        {activeTab === 'import' && (
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                                ...
                                <p className="text-slate-500 text-sm mb-6">Upload a PDF or paste text (lecture notes, paragraphs, etc.) and let AI generate questions for you.</p>

                                {previewQuestions.length === 0 ? (
                                    <div className="space-y-4">
                                        {/* PROVIDER SELECTOR */}
                                        <div className="bg-slate-50 p-2 rounded-lg flex items-center justify-center gap-4 mb-4 border border-slate-200">
                                            <label className="flex items-center gap-2 cursor-pointer">
                                                ...
                                                />
                                                <span className={`text-sm font-bold ${provider === 'gemini' ? 'text-purple-700' : 'text-slate-500'}`}>Google Gemini</span>
                                            </label>
                                            ...
                                            <label className="flex items-center gap-2 cursor-pointer">
                                                ...
                                                />
                                                <span className={`text-sm font-bold ${provider === 'groq' ? 'text-orange-700' : 'text-slate-500'}`}>Groq (Llama 3)</span>
                                            </label>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className={`w-full p-3 rounded-lg border border-slate-300 focus:ring-2 outline-none font-mono text-sm ${provider === 'gemini' ? 'focus:ring-purple-500' : 'focus:ring-orange-500'
                                                    }`}
                                                value={apiKey}
                                        ...
                                        {/* PDF UPLOAD */}
                                        <div>
                                            <label className="block text-sm font-bold text-slate-700 mb-1">Use a Document (PDF)</label>
                                            <div className="flex items-center gap-4">
                                                <label className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 p-2 rounded-lg cursor-pointer border border-slate-300 text-slate-600 font-medium transition-all">
                                                ...
                                                {importText && <span className="text-xs text-green-600 font-bold flex items-center gap-1"><Check className="w-3 h-3" /> Ready</span>}
                                            </div>
                                        </div>

                                        <div>
                                            <label className="block text-sm font-bold text-slate-700 mb-1">Content Text (Editable)</label>
                                            <textarea
                                                className={`w-full p-3 rounded-lg border border-slate-300 focus:ring-2 outline-none h-48 font-mono text-sm ${provider === 'gemini' ? 'focus:ring-purple-500' : 'focus:ring-orange-500'
                                                    }`}
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <button
                                                ...
                                                disabled={!importText}
                                                className="bg-slate-200 text-slate-700 font-bold py-3 rounded-xl hover:bg-slate-300 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                                            >
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center bg-purple-50 p-4 rounded-xl border border-purple-100">
                                            <span className="font-bold text-purple-800">Generated {previewQuestions.length} Questions</span>
                                            <div className="flex gap-2">
                                                <button
                                                    onClick={() => setPreviewQuestions([])}
                                                    className="px-4 py-2 text-slate-600 hover:bg-white rounded-lg transition-colors border border-transparent hover:border-slate-300"
                                                >
                                                    Discard
                                                </button>
                                                <button
                                                    onClick={confirmImport}
                                                    className="px-6 py-2 bg-purple-600 text-white font-bold rounded-lg hover:bg-purple-700 shadow-lg shadow-purple-200"
                                                >
                                                    Save All
                                                </button>
                                            </div>
                                            ...
                                            {previewQuestions.map((q, i) => (
                                                <div key={i} className="border border-slate-200 rounded-xl p-4 bg-slate-50">
                                                    <div className="flex justify-between mb-2">
                                                        <span className="text-xs font-bold bg-slate-200 text-slate-600 px-2 py-1 rounded">{q.theme}</span>
                        ...
                        {activeTab === 'list' && (
                            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                                <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                                    <h2 className="font-bold text-slate-700">All Questions</h2>
                                ...
                                <div className="divide-y divide-slate-100 max-h-[600px] overflow-y-auto">
                                    {questions.filter(q =>
                                        ...
                                        <div key={q.id} className="p-4 hover:bg-slate-50 transition-colors flex justify-between gap-4 group">
                                            <div className="flex-1">
                                                ...
                                                <p className="text-slate-800 font-medium text-sm line-clamp-2">{q.question}</p>
                                            </div>
